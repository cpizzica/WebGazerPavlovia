...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': 'gazeConditions.xlsx', 'path': 'resources/gazeConditions.xlsx'}
  ]
});
...
function StartTask_3RoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of StartTask_3Components)
...
function EnterDistanceRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of EnterDistanceComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function EnterHeightRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of EnterHeightComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function WaitForCalibrationRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of WaitForCalibrationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function CalibrateGazeTrackerRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of CalibrateGazeTrackerComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function RandomizePositionLoopBegin(RandomizePositionLoopScheduler) {
  ...
  // Schedule all the trials in the trialList:
  for (const thisRandomizePosition of RandomizePosition) {
...
function ShowGazeTargetRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of ShowGazeTargetComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function WaitForNextTrialRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of WaitForNextTrialComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
